...
export default function HomeTab({ onSelectTemplate, onSaveTemplate, user }: HomeTabProps) {
  ...
  return (
    <div 
      className="bg-slate-50 text-slate-800 w-full h-full overflow-x-hidden overflow-y-auto custom-scrollbar font-sans selection:bg-indigo-500/20 relative flex flex-col items-center pb-24"
      onMouseMove={handleMouseMove}
    >
      {/* Decorative Grid Mesh Background */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f0_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-[0.35] pointer-events-none z-0"></div>
      ...
      <div className="relative py-12 md:py-20 flex flex-col items-center text-center px-4 w-full max-w-5xl z-10">
        ...
        <div 
          className="absolute inset-0 pointer-events-none transition-opacity duration-1000 opacity-80 md:opacity-100 z-0 overflow-hidden rounded-[2.5rem]"
          style={{
            filter: 'blur(100px)',
          }}
        >
        ...
        <div className="relative z-10 mb-8 mt-4">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 border border-indigo-100/60 rounded-full text-xs font-bold text-indigo-600 mb-6 uppercase tracking-wider animate-fade-in shadow-sm">
            <Sparkles className="w-3.5 h-3.5" /> Workspace Smart AI Generator
          </span>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-5 max-w-3xl">
          ...
          <p className="text-[15px] md:text-base font-semibold text-slate-500 max-w-2xl leading-relaxed mx-auto">
            Hóa giải trạng thái mơ hồ bằng cách chia nhỏ mục tiêu thành khung sườn Multi-Block vững chắc đối kháng ảo giác AI. Nhập chủ đề của bạn dưới đây.
          </p>
        </div>

        {/* Luồng tạo prompt dưới dạng thanh tìm kiếm AI thông minh */}
        <motion.div 
          className="w-full max-w-3xl bg-white/90 backdrop-blur-xl border border-slate-200 shadow-[0_20px_50px_rgba(15,23,42,0.06)] rounded-[2.5rem] p-3 md:p-4 z-20 relative group transition-all hover:border-slate-300"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.5 }}
        >
          {/* Subtle liquid outline glow matching "làn nước" concept */}
          <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500/10 via-sky-500/10 to-emerald-500/10 rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition duration-700 blur-[6px] -z-10 pointer-events-none" />
          ...
          <form onSubmit={handleGeneratePrompt} className="relative flex flex-col md:flex-row items-stretch md:items-center gap-2.5">
            <div className="relative flex-1 flex items-center">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 bg-transparent border-0 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-0 text-medium md:text-[15px] font-semibold"
              />
            </div>
            <button 
              type="submit"
              disabled={isGenerating || !searchInput.trim()}
              className="py-3.5 px-6 rounded-2xl bg-slate-900 text-white font-extrabold text-sm flex items-center justify-center gap-2 transition-all hover:bg-indigo-600 hover:shadow-lg active:scale-95 disabled:bg-slate-200 disabled:text-slate-400 disabled:scale-100"
            >
              {isGenerating ? (
                <>
          ...
          {/* Suggestions List */}
          <div className="mt-4 pt-3 border-t border-slate-100/80 flex flex-wrap gap-2 items-center text-left">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest pl-1">Gợi ý từ AI:</span>
            {POPULAR_SUGGESTIONS.map((suggestion, index) => (
              <button
                key={index}
                onClick={() => setSearchInput(suggestion.text)}
                type="button"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200/60 rounded-xl text-xs text-slate-600 font-semibold hover:border-indigo-200 hover:bg-indigo-50/50 hover:text-indigo-600 transition-all cursor-pointer"
              >
        ...
        <AnimatePresence>
          {isGenerating && (
            <motion.div 
              ...
              className="w-full max-w-2xl mt-8 bg-indigo-50/50 border border-indigo-100/60 rounded-3xl p-6 text-center shadow-sm overflow-hidden"
            >
              <div className="flex flex-col items-center gap-4">
                {/* Micro spinning water circles */}
                ...
                <div className="flex flex-col gap-1.5 w-full">
                  <span className="text-[13px] font-bold text-indigo-700 animate-pulse">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">Bố cục Multi-block đang cấu hình cho tối ưu ngữ cảnh</span>
                </div>
              </div>
            </motion.div>
          )}
        ...
        {errorMessage && (
          <div className="w-full max-w-2xl mt-6 p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-700 text-xs font-semibold text-center flex items-center justify-center gap-2">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-700 uppercase tracking-widest">KẾT QUẢ PROMPT HOÀN CHỈNH</span>
                </div>
              ...
              <div className="bg-white border border-slate-200/80 shadow-[0_15px_40px_rgba(0,0,0,0.04)] rounded-3xl overflow-hidden relative">
                {/* Soft gradient header inside card */}
                <div className="p-6 md:p-8 bg-gradient-to-r from-slate-50 to-indigo-50/30 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      <span className="px-2 py-0.5 bg-indigo-50 border border-indigo-100/50 rounded-md text-[9px] font-black uppercase text-indigo-600 tracking-wider">
                        {generatedTemplate.category || 'Mẫu của tôi'}
                      </span>
                      {generatedTemplate.tags?.map((tag: string, index: number) => (
                        <span key={index} className="px-2 py-0.5 bg-slate-100 rounded-md text-[9px] font-bold text-slate-500">
                          #{tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:self-center">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-slate-50 text-slate-700 hover:text-slate-900 border border-slate-200 hover:border-slate-300 font-extrabold text-xs flex items-center gap-1.5 transition-all cursor-pointer active:scale-95"
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 bg-indigo-600 hover:bg-slate-905 text-white font-extrabold text-xs rounded-xl flex items-center gap-1.5 shadow-md shadow-indigo-200/50 hover:shadow-lg transition-all cursor-pointer active:scale-95"
                    >
                ...
                <div className="divide-y divide-slate-100 bg-white">
                  {generatedTemplate.blocks?.map((block: any) => (
                    <div key={block.id} className="p-5 md:p-6 flex flex-col md:flex-row gap-4 md:items-start hover:bg-slate-50/40 transition-colors">
                      {/* Left Block Identifier tag */}
                      <div className="w-full md:w-44 shrink-0 flex items-center gap-1.5 md:flex-col md:items-stretch">
                        <span className="text-[10px] font-black text-rose-500/80 font-mono uppercase tracking-widest py-0.5 bg-rose-50 border border-rose-100 px-2 rounded-md self-start">
                          {block.type}
                        </span>
                      ...
                      <div className="flex-1 bg-slate-50/60 border border-slate-100 p-4 rounded-xl font-mono text-[11px] md:text-xs text-slate-600 whitespace-pre-wrap leading-relaxed shadow-inner">
                        {block.content}
                      </div>
                    </div>
                  ))}
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center gap-2 text-[10px] text-slate-400 font-semibold px-6">
